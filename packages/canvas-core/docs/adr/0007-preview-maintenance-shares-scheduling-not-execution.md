...
Folder Disclosure determines ordinary membership in active Workbench
maintenance. A video with a persisted Feedback Moment is the sole exception:
its exact Moment targets remain maintenance members through Runtime's separate
lightweight Feedback-video view, while the hidden path has no Canvas node or
playback target. Hiding any other node removes its current target, every task that has not crossed its
...
The persisted Feedback-video exception has no visible node on which to present
its target-local read, decode, capture, or save failure. Its exact failed target
therefore stays quiescent only inside the current Video registry and creates no
Activity or automatic retry. A new Workbench registry, changed Source Revision,
or changed Feedback Moment creates a new attempt. Redisclosure also explicitly
retries failed targets for that path before they can participate in visible
preview state, so a hidden failure is never adopted as the player's error.

Text keeps its DOM-capture registry and Video owns a separate exact-target
registry. Video source availability is one bounded Runtime read while source
generation uses one serialized Workbench browser capture and Runtime save. One
Project path may retain multiple independent playback and Feedback Moment
targets. Persisted Feedback-video targets use the same registry, source resolver,
and capture lane rather than a second maintenance implementation. A task ends when its type-specific source is confirmed. Width selection,
Runtime raster variants and cache, resource-start
...
pixel-affecting target inputs into a Preview Target Identity. Requested width
then produces a Preview Variant Identity. Project binding and
...
uses the Source Revision plus exact frame time. A continuity change
...
width selection and mounted presentation while keeping source producers
