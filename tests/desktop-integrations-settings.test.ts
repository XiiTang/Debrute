...
import { IntegrationsSettingsPage } from '../apps/desktop/src/workbench/settings/integrations/IntegrationsSettingsPage';
import type { WorkbenchActions, WorkbenchState } from '../apps/desktop/src/types';

describe('desktop Integrations settings page', () => {
  ...
  it('renders ready, missing, failed, and Python CLI integration states', () => {
    ...
    expect(html).not.toContain('FFmpeg&amp;ffprobe');
    ...
    expect(html).not.toContain('Video and audio processing toolkit.');
    expect(html).not.toContain('<p>Ready.</p>');
  });

  it('renders integration rows without path UI or redundant FFmpeg companion binary labels', () => {
    const html = renderToStaticMarkup(React.createElement(IntegrationsSettingsPage, {
      state: createState(),
      actions: createActions()
    }));

    expect(html).toContain('<div class="integration-row"><span>FFmpeg</span>');
    ...
    expect(html).not.toContain('<span>ffmpeg</span>');
    expect(html).not.toContain('<span>ffprobe</span>');
    expect(html).not.toContain('/opt/homebrew/bin/ffmpeg');
...
function createActions(): WorkbenchActions {
  return {
    refreshIntegrationsStatus: async () => createState().integrationsSettings!,
