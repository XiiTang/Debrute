...
import { runCli } from '../apps/axis-cli/src/index';

describe('axis-cli', () => {
  it('initializes, reports, and validates projects with axis/1 records', async () => {
    const root = await mkdtemp(join(tmpdir(), 'axis-cli-project-'));
    const home = await mkdtemp(join(tmpdir(), 'axis-cli-home-'));
    ...
    try {
      ...
      expect(output[0]?.split('\n')[0]).toBe('axis/1 ok cmd=project.init');
      expect(output[1]).toContain('axis/1 ok cmd=project.status');
      expect(output[1]).toContain('project_name=');
      expect(output[2]).toContain('axis/1 ok cmd=project.validate');
  ...
  it('does not initialize missing projects for read-only project commands', async () => {
    const root = await mkdtemp(join(tmpdir(), 'axis-cli-readonly-noinit-'));
    const originalExitCode = process.exitCode;
    try {
      const output: string[] = [];
      ...
      expect(output).toEqual([expect.stringMatching(/^axis\/1 error cmd=project\.status code=project_not_found\nmessage=/)]);
      await expect(readFile(join(root, '.axis/project.json'), 'utf8')).rejects.toThrow();
    } finally {
      process.exitCode = originalExitCode;
      await rm(root, { recursive: true, force: true });
    }
  ...
  it('publishes Flowmaps through flowmap publish', async () => {
    const root = await mkdtemp(join(tmpdir(), 'axis-cli-flowmap-publish-'));
    const originalExitCode = process.exitCode;
    try {
      await runCli(['project', 'init', root], () => {});
      await mkdir(join(root, '.axis/flowmaps'), { recursive: true });
      await writeFile(join(root, '.axis/flowmaps/image-production.draft.yaml'), [
        'schemaVersion: 1',
        'canvases:',
        '  - production-map',
        'include: []',
      ...
      await runCli([
        ...
        root,
        '--from',
        '.axis/flowmaps/image-production.draft.yaml'
      ], (text) => output.push(text));

      expect(output).toEqual(['axis/1 ok cmd=flowmap.publish\nsource=.axis/flowmaps/image-production.draft.yaml']);
      await expect(readFile(join(root, '.axis/flowmaps/image-production.yaml'), 'utf8')).resolves.toContain('contentHash: sha256:');
    } finally {
      process.exitCode = originalExitCode;
      await rm(root, { recursive: true, force: true });
    }
  ...
  it('returns Flowmap-specific axis/1 errors', async () => {
    const root = await mkdtemp(join(tmpdir(), 'axis-cli-flowmap-error-'));
    const originalExitCode = process.exitCode;
    try {
      await runCli(['project', 'init', root], () => {});
      ...
      await runCli([
        ...
        root,
        '--from',
        '.axis/flowmaps/new-map.draft.yaml'
      ], (text) => output.push(text));

      expect(process.exitCode).toBe(1);
      expect(output).toEqual([[
        'axis/1 error cmd=flowmap.publish code=flowmap_draft_read_failed',
        'message="Flowmap draft could not be read."',
        'file_path=.axis/flowmaps/new-map.draft.yaml'
      ].join('\n')]);
      await expect(readFile(join(root, '.axis/flowmaps/new-map.yaml'), 'utf8')).rejects.toThrow();
    } finally {
      process.exitCode = originalExitCode;
      await rm(root, { recursive: true, force: true });
    }
  ...
  it('uses runtime-scoped model and llm commands without a project', async () => {
    const home = await mkdtemp(join(tmpdir(), 'axis-cli-runtime-home-'));
    const originalHome = process.env.HOME;
    const originalExitCode = process.exitCode;
    try {
      process.env.HOME = home;
      ...
      expect(output[0]?.split('\n')[0]).toBe('axis/1 ok cmd=runtime.status');
      expect(output[1]).toContain('axis/1 ok cmd=models.image.list');
      expect(output[1]).toContain('count=0');
      expect(output[2]).toContain('axis/1 ok cmd=models.video.list');
      expect(output[2]).toContain('model id=');
      expect(output[3]).toContain('axis/1 error cmd=llm.request code=model_not_configured');
  ...
  it('prints configured image model parameter summaries without config status fields', async () => {
    const home = await mkdtemp(join(tmpdir(), 'axis-cli-image-list-params-home-'));
    const originalHome = process.env.HOME;
    const originalExitCode = process.exitCode;
    try {
      process.env.HOME = home;
      await mkdir(join(home, '.axis/config'), { recursive: true });
      await writeFile(join(home, '.axis/config/secrets.json'), JSON.stringify({
        llmProviderApiKeys: {},
  ...
  it('returns stable user-facing errors for unknown model descriptions', async () => {
    ...
    try {
      ...
      expect(output).toEqual([[
        'axis/1 error cmd=models.image.describe code=model_unavailable',
  ...
  it('prints official image model documentation in model describe output', async () => {
    ...
    try {
      ...
      expect(output[0]).toContain('axis generate image <project> --input-json');
  ...
  it('returns configuration exit codes for known generation models without config', async () => {
    const root = await mkdtemp(join(tmpdir(), 'axis-cli-generate-config-'));
    const home = await mkdtemp(join(tmpdir(), 'axis-cli-generate-config-home-'));
    const originalHome = process.env.HOME;
    const originalExitCode = process.exitCode;
    try {
      process.env.HOME = home;
  ...
  it('returns configuration exit codes when generation model API keys are missing', async () => {
    const root = await mkdtemp(join(tmpdir(), 'axis-cli-generate-auth-'));
    const home = await mkdtemp(join(tmpdir(), 'axis-cli-generate-auth-home-'));
    const originalHome = process.env.HOME;
    const originalExitCode = process.exitCode;
    try {
      process.env.HOME = home;
      await runCli(['project', 'init', root], () => {});
      await mkdir(join(home, '.axis/config'), { recursive: true });
      await writeFile(join(home, '.axis/config/image_models.json'), JSON.stringify({
        imageModels: [{
  ...
  it('looks up generated asset metadata through a project command', async () => {
    const root = await mkdtemp(join(tmpdir(), 'axis-cli-generated-asset-'));
    const originalExitCode = process.exitCode;
    try {
      await runCli(['project', 'init', root], () => {});
      ...
      expect(output).toEqual([expect.stringContaining('axis/1 ok cmd=generated-asset.lookup\nstatus=unavailable\nreason=missing')]);
    } finally {
      process.exitCode = originalExitCode;
      await rm(root, { recursive: true, force: true });
    }
  ...
  it('prints command specs instead of human help pages', async () => {
    ...
    expect(output[1]).toContain('axis/1 ok cmd=help');
  ...
  it('rejects --json', async () => {
    ...
    try {
      ...
      expect(jsonOutput).toEqual([[
        'axis/1 error cmd=project.status code=invalid_argument',
