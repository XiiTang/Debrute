...
const serverInstances = vi.hoisted(() => [] as FakeAxisAppServer[]);

vi.mock('@axis/app-server', () => {
  class AxisAppServer {
    ...
    constructor() {
      serverInstances.push(this as unknown as FakeAxisAppServer);
    }
  }
  return { AxisAppServer };
});
import { runCli } from '../apps/axis-cli/src/index';

interface FakeAxisAppServer {
...
describe('axis cli image generation batch', () => {
  beforeEach(() => {
  ...
  test('opens the project once and delegates image batches to app-server', async () => {
    const tempDir = await mkdtemp(join(tmpdir(), 'axis-cli-batch-'));
    ...
    try {
      ...
      expect(lines).toEqual([[
        'axis/1 ok cmd=generate.image-batch',
        'total=2',
        'ok=2',
        'failed=0',
        'skipped=0',
  ...
  test('passes manifest batch sources to app-server without CLI expansion', async () => {
    const tempDir = await mkdtemp(join(tmpdir(), 'axis-cli-manifest-'));
    const projectRoot = join(tempDir, 'project');
    await mkdir(projectRoot);
  ...
  test('requires a log path for image batches before creating the app-server', async () => {
    ...
    expect(lines).toEqual([[
      'axis/1 error cmd=generate.image-batch code=missing_argument',
