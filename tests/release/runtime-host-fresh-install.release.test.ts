...
describe('runtime host fresh install product startup', () => {
  ...
  it('awaits managed CLI materialization before the daemon listens', async () => {
    ...
    const productUpdate = {
      state: vi.fn(async () => ({
        ...
        update: { type: 'idle' as const, currentVersion: '0.2.0', updateAvailable: false as const }
      })),
      check: vi.fn(),
      apply: vi.fn()
    };
  ...
  it('creates product updates with the managed helper command outside the desktop install', async () => {
    ...
    vi.mocked(spawn).mockReturnValueOnce(spawned as unknown as ReturnType<typeof spawn>);
