...
describe('Debrute managed CLI runtime packaging', () => {
  ...
  it('filters CLI node-pty payload to the target prebuild and runtime JavaScript files', () => {
    ...
    try {
      ...
      const libFilter = libEntry && 'filter' in libEntry
        ? libEntry.filter as (source: string) => boolean
        : undefined;
      const prebuildFilter = prebuildEntry && 'filter' in prebuildEntry
        ? prebuildEntry.filter as (source: string) => boolean
        : undefined;
      ...
      expect(libFilter?.(join(root, 'node_modules/node-pty/lib/index.js'))).toBe(true);
      expect(libFilter?.(join(root, 'node_modules/node-pty/lib/terminal.test.js'))).toBe(false);
      expect(libFilter?.(join(root, 'node_modules/node-pty/lib/windowsTerminal.js'))).toBe(false);
      expect(prebuildFilter?.(join(root, 'node_modules/node-pty/prebuilds/darwin-arm64/pty.node'))).toBe(true);
      expect(prebuildFilter?.(join(root, 'node_modules/node-pty/prebuilds/darwin-arm64/spawn-helper'))).toBe(true);
      expect(prebuildFilter?.(join(root, 'node_modules/node-pty/prebuilds/darwin-arm64/pty.pdb'))).toBe(false);
    } finally {
      rmSync(root, { recursive: true, force: true });
    }
  });
