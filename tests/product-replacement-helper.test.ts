...
import { nodeProductReplacementHelperOperations, runProductReplacementHelper } from '../apps/runtime-host/src/productReplacementHelper';
import type { ProductReplacementPlan } from '../apps/daemon/src/product/ProductReplacementPlan';

const execFile = promisify(execFileCallback);

describe('product replacement helper', () => {
  it('is importable without running a plan from the caller argv', async () => {
    const pnpm = process.platform === 'win32' ? 'pnpm.cmd' : 'pnpm';
    const script = [
      'void (async () => {',
      "process.argv[1] = '/tmp/debrute-managed-cli';",
      "process.argv[2] = 'internal-product-replacement-helper';",
      "await import('./apps/runtime-host/src/productReplacementHelper.ts');",
      "console.log('loaded');",
      '})()'
    ].join(' ');

    await expect(execFile(pnpm, ['exec', 'tsx', '-e', script], {
      cwd: process.cwd()
