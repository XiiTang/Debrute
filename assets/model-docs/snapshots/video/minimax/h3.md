...
downloaded immediately into the Project rather than retained as a durable
remote URL.
...
As a Project extension, an existing recognizable Project-relative media file
in the same native `.url` field is read under Debrute's resource limits and
converted to a data URI. Other strings remain unchanged for MiniMax to judge.
