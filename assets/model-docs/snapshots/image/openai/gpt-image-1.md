...
Currently documented `background` values include `transparent`, `opaque`, and
`auto`; documented `input_fidelity` values include `low` and `high`. Debrute
materializes no default for this Model. OpenAI owns current enum, range,
cardinality, and cross-field validation.
