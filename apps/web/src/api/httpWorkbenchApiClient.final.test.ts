...
describe('Runtime Workbench connection', () => {
  it('uses one connection credential for commands and never puts it in a URL', async () => {
    ...
    const client = createHttpWorkbenchApiClient(harness.options);
  ...
  it('does not reconnect after the Runtime connection ends', async () => {
    ...
    await expect(client.getProductState()).rejects.toThrow('ended unexpectedly');
...
function createHarness() {
  ...
  const fetchImpl = vi.fn(async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    ...
    if (path === '/api/runtime/product') {
      return Response.json({ version: '0.0.3' });
    }
