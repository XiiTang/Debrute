...
describe('canvas video preview URLs', { tags: ['canvas-video'] }, () => {
  it('builds video preview URLs from one owner-scoped target', () => {
    ...
    expect(request.variantTarget?.srcForWidth(300)).toBe(
      '/api/workbench/bindings/123e4567-e89b-42d3-a456-426614174000/canvas-video-preview?path=media%2Fclip.mp4&sourceRevision=rev-video&frameTimeMs=0&canonicalSourceIdentity=frame-v1--ms-0&w=300'
    );
  });
});
