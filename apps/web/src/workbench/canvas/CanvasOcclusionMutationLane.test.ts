...
describe('Canvas Occlusion Mutation Lane', () => {
  it('runs one mutation per Canvas at a time and lets each read the preceding result', async () => {
    const lane = createCanvasOcclusionMutationLane();
    const first = deferred<void>();
    let acceptedOrder = 'base';
    const observations: string[] = [];

    const firstMutation = lane.run('canvas-1', async () => {
      observations.push(acceptedOrder);
      await first.promise;
      acceptedOrder = 'after-first';
    });
    const secondMutation = lane.run('canvas-1', async () => {
      observations.push(acceptedOrder);
  ...
  it('reports a failed mutation without blocking the next one', async () => {
    ...
    const failed = lane.run('canvas-1', async () => {
      throw new Error('write failed');
    });
    const succeeded = lane.run('canvas-1', next);
