...
describe('CanvasTextEditorRuntime', { tags: ['canvas-text'] }, () => {
  ...
  it('leaves Escape behavior to the CodeMirror search and default keymaps', () => {
    const callbacks: CanvasTextEditorCallbackRef = {
      current: {
        onChange: vi.fn(),
        onSave: vi.fn(),
    ...
    const expectedEscapeBindings = [...searchKeymap, ...defaultKeymap].filter((item) => item.key === 'Escape');
    const actualEscapeBindings = canvasTextEditorKeymap(callbacks).filter((item) => item.key === 'Escape');

    expect(actualEscapeBindings).toEqual(expectedEscapeBindings);
  });
