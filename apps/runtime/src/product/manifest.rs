...
impl ProductManifest {
    pub(crate) fn validate_contract(&self) -> Result<(), ProductManifestError> {
        ...
        let expected_entrypoints = match self.platform {
        ...
        for entrypoint in [
            ...
            self.entrypoints.web.as_str(),
            self.entrypoints.cli.as_str(),
            self.entrypoints.skills.as_str(),
            self.entrypoints.native_workers.as_str(),
        ] {
