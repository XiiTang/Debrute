...
use super::{
    CommitPhase, InstalledProductLayout, ProductLayoutError, ProductProjectionError,
    ProductProjectionManager, ProductStore, ProductStoreError,
};
...
pub struct ProductInstallationCoordinator {
    store: Arc<ProductStore>,
    layout: InstalledProductLayout,
}

impl ProductInstallationCoordinator {
    #[must_use]
    pub fn new(store: Arc<ProductStore>, layout: InstalledProductLayout) -> Self {
        Self { store, layout }
    }
    ...
    /// Performs an initial installation or repairs the already-selected same
    /// Product version, then converges all user-facing projections.
    ///
    /// # Errors
    ///
    /// Returns [`ProductInstallationError`] when any Product, stable entrypoint,
    /// Skills, or Desktop registration boundary is invalid.
    pub fn install_initial_or_repair(
        &self,
        seed: &Path,
        desktop: Option<&DesktopHostRegistration>,
    ) -> Result<ActivatedProduct, ProductInstallationError> {
        if self.store.pending()?.is_some() {
            return Err(ProductInstallationError::PendingCommitExists);
        }
        let seed_identity = self.store.inspect_seed_identity(seed)?;
        if let Some(current) = self.store.current_version()?
            && current != seed_identity.product_version()
        {
            return Err(ProductInstallationError::ProductVersionRequiresCommit {
                current,
                target: seed_identity.product_version().to_owned(),
            });
        }
        let directory = self.store.activate_desktop_seed(seed)?;
        self.finalize_product(directory, desktop)
    }

    /// Resolves the one Runtime executable allowed to continue an existing
    /// Product update without mutating Product selection or projections.
    ///
    /// # Errors
    ///
    /// Returns [`ProductInstallationError`] when no pending commit exists or
    /// the embedded seed is not the exact old or target Product authorized by
    /// that transaction.
    pub fn recover_pending(
        ...
        let pending = self
            .store
            .pending()?
            .ok_or(ProductInstallationError::PendingCommitMissing)?;
        let seed_identity = self.store.inspect_seed_identity(seed)?;
        ...
        if seed_version != pending.target_version && !recover_old_runtime {
            return Err(ProductInstallationError::PendingTargetMismatch {
                expected: pending.target_version,
                actual: seed_version.to_owned(),
            });
        }
        self.store.validate_version(seed_version)?;
        Ok(self.activated_product(self.store.version_path(seed_version)))
    }
    ...
    /// # Errors
    ///
    /// Returns [`ProductInstallationError`] if current is missing or any selected
    ...
    pub fn finalize_current(
        ...
        let version = self
            .store
            .current_version()?
            .ok_or(ProductInstallationError::CurrentProductMissing)?;
    ...
    fn finalize_product(
        ...
        if self.store.root() != self.layout.product_root() {
            return Err(ProductInstallationError::LayoutMismatch);
        }
        ...
        ProductProjectionManager::configure_command_path(&self.layout)?;
        if let Some(desktop) = desktop {
            if desktop.executable != self.layout.desktop_executable()
                || !desktop.arguments.is_empty()
            {
                return Err(ProductInstallationError::InvalidDesktopRegistration(
                    "Desktop registration must use the exact installed Product path without arguments"
                        .to_owned(),
                ));
            }
            write_desktop_host_registration(self.layout.debrute_home(), desktop)?;
        }
        Ok(ActivatedProduct {
            product_version: directory
                .file_name()
                .and_then(|name| name.to_str())
                .ok_or_else(|| ProductInstallationError::InvalidEntrypoint(directory.clone()))?
                .to_owned(),
...
/// Reads the one closed Desktop launch registration written by Product installation.
///
/// # Errors
///
/// Returns [`ProductInstallationError`] when an existing registration is malformed.
pub fn read_desktop_host_registration(
    debrute_home: &Path,
) -> Result<Option<DesktopHostRegistration>, ProductInstallationError> {
    ...
    let file: DesktopHostRegistrationFile = serde_json::from_slice(&bytes)
        .map_err(|error| ProductInstallationError::InvalidDesktopRegistration(error.to_string()))?;
...
) -> Result<(), ProductInstallationError> {
    validate_desktop_registration(registration)?;
...
fn install_runtime_entrypoint(
    ...
    let application = current_runtime
        ...
        .and_then(Path::parent)
        .ok_or_else(|| ProductInstallationError::InvalidEntrypoint(current_runtime.to_owned()))?;
...
#[cfg(target_os = "macos")]
fn shell_escaped_path(path: &Path) -> Result<String, ProductInstallationError> {
    path.to_str()
        .map(|path| path.replace('\'', "'\\''"))
        .ok_or_else(|| ProductInstallationError::InvalidEntrypoint(path.to_owned()))
}

#[cfg(target_os = "macos")]
fn write_unix_script(destination: &Path, source: &str) -> Result<(), ProductInstallationError> {
...
fn write_file_atomic(destination: &Path, bytes: &[u8]) -> Result<(), ProductInstallationError> {
    let parent = destination
        .parent()
        .ok_or_else(|| ProductInstallationError::InvalidEntrypoint(destination.to_owned()))?;
...
#[cfg(target_os = "macos")]
fn set_private_file_permissions(path: &Path) -> Result<(), ProductInstallationError> {
    use std::os::unix::fs::PermissionsExt as _;
...
#[derive(Debug)]
pub enum ProductInstallationError {
    ...
    ProductVersionRequiresCommit { current: String, target: String },
    LayoutMismatch,
}

impl fmt::Display for ProductInstallationError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(formatter, "Product installation failed: {self:?}")
    }
}

impl Error for ProductInstallationError {
...
impl From<ProductStoreError> for ProductInstallationError {
    fn from(error: ProductStoreError) -> Self {
        Self::Product(error)
    }
}

impl From<ProductProjectionError> for ProductInstallationError {
    fn from(error: ProductProjectionError) -> Self {
        Self::Projection(error)
    }
}

impl From<ProductLayoutError> for ProductInstallationError {
    fn from(error: ProductLayoutError) -> Self {
        Self::Layout(error)
    }
}

impl From<io::Error> for ProductInstallationError {
...
mod tests {
    ...
    fn desktop_seed_installs_stable_entrypoints_skills_and_closed_desktop_registration() {
        let root = std::env::temp_dir().join(format!("debrute-installation-{}", Uuid::new_v4()));
        ...
        let installation = ProductInstallationCoordinator::new(
        ...
        let desktop = DesktopHostRegistration {
            executable: layout.desktop_executable().to_owned(),
            arguments: Vec::new(),
        };

        let activated = installation
            .install_initial_or_repair(&seed, Some(&desktop))
            .unwrap();
    ...
    #[test]
    fn product_installation_requires_the_update_commit_to_advance_versions() {
        let root = std::env::temp_dir().join(format!("debrute-installation-{}", Uuid::new_v4()));
        let home = root.join("home");
        let layout = InstalledProductLayout::for_roots(CommitPlatform::Macos, home, None).unwrap();
        let product_root = layout.product_root().to_owned();
        let installation = ProductInstallationCoordinator::new(
            Arc::new(ProductStore::new(
                product_root.clone(),
                CommitPlatform::Macos,
        ...
        installation
            .install_initial_or_repair(&write_seed(&root, "0.0.3"), None)
            .unwrap();
        let advance = installation
            .install_initial_or_repair(&write_seed(&root, "0.0.4"), None)
            .unwrap_err();
        let downgrade = installation
            .install_initial_or_repair(&write_seed(&root, "0.0.2"), None)
            .unwrap_err();

        assert!(matches!(
            advance,
            ProductInstallationError::ProductVersionRequiresCommit { .. }
        ));
        assert!(matches!(
            downgrade,
            ProductInstallationError::ProductVersionRequiresCommit { .. }
