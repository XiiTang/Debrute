...
use super::types::{
    HttpMethod, HttpTargetPolicy, ModelHttpRequest, ModelHttpResponse, ModelHttpTransport,
    ModelRequestCancellation, ModelRequestDeadline, ModelRequestError, PreparedHttpBody,
};
            ...
            scope
                .spawn(|| {
                    tokio::runtime::Builder::new_current_thread()
                        ...
                        .build()
                        .map_err(|error| {
                            ModelRequestError::new("model_request_failed", error.to_string())
                        })?
                        .block_on(async {
                ...
                .join()
                .expect("Model request HTTP worker panicked")
        })
    }
}
...
async fn execute_public(
    ...
    for redirect in 0..=MAX_PUBLIC_REDIRECTS {
        ...
        request.url = url
            .join(location)
            .map_err(|error| {
                ModelRequestError::new("remote_media_redirect_invalid", error.to_string())
            })?
            .to_string();
...
async fn execute_once(
    ...
    let client = builder
        .build()
        .map_err(|error| ModelRequestError::new("model_request_failed", error.to_string()))?;
    ...
    loop {
        let chunk = cancellable(response.chunk(), cancellation, deadline)
            .await?
            .map_err(|error| ModelRequestError::new("model_response_failed", error.to_string()))?;
...
fn outbound_headers(request: &ModelHttpRequest) -> Result<HeaderMap, ModelRequestError> {
...
) -> Result<reqwest::RequestBuilder, ModelRequestError> {
    Ok(match body {
        ...
        PreparedHttpBody::Multipart { fields, files } => {
            ...
            for file in files {
                let part = multipart::Part::bytes(file.bytes)
                    ...
                    .map_err(|error| {
                        ModelRequestError::new("model_request_invalid", error.to_string())
                    })?;
...
fn parse_model_url(url: &str) -> Result<url::Url, ModelRequestError> {
    let url = url::Url::parse(url)
        .map_err(|error| ModelRequestError::new("model_request_invalid", error.to_string()))?;
...
pub(crate) fn parse_public_url(url: &str) -> Result<url::Url, ModelRequestError> {
    let url = url::Url::parse(url)
        .map_err(|error| ModelRequestError::new("remote_media_url_invalid", error.to_string()))?;
    ...
    url.host_str().ok_or_else(|| {
        ModelRequestError::new("remote_media_url_invalid", "Remote media URL has no host.")
    })?;
    url.port_or_known_default().ok_or_else(|| {
        ModelRequestError::new("remote_media_url_invalid", "Remote media URL has no port.")
    })?;
    Ok(url)
}
...
pub(crate) fn validate_public_url(
    ...
    cancellation: &ModelRequestCancellation,
    deadline: ModelRequestDeadline,
) -> Result<url::Url, ModelRequestError> {
...
fn resolve_url_addresses(
    ...
    deadline: ModelRequestDeadline,
    require_public: bool,
) -> Result<Vec<SocketAddr>, ModelRequestError> {
...
fn bounded_resolve(
    ...
    std::thread::Builder::new()
        .name("debrute-model-request-dns".to_owned())
        .spawn(move || {
            let _slot = slot;
        ...
        .map_err(|error| ModelRequestError::new("remote_media_dns_failed", error.to_string()))?;
    loop {
        let remaining = deadline.remaining(cancellation)?;
...
impl DnsSlot {
    fn acquire() -> Result<Self, ModelRequestError> {
...
fn header_map(headers: &BTreeMap<String, String>) -> Result<HeaderMap, ModelRequestError> {
...
fn map_request_error(request_timeout: Duration, error: &reqwest::Error) -> ModelRequestError {
    if error.is_timeout() {
        ModelRequestError::new(
...
mod tests {
    ...
    fn request_bodies_are_bounded_before_network_access() {
        let body = PreparedHttpBody::Multipart {
            fields: BTreeMap::new(),
            files: vec![crate::model_request::types::MultipartFile {
                name: "image".to_owned(),
    ...
    fn stalled_http_is_interrupted_by_cancellation() {
        ...
        let error = NativeModelHttpTransport
            ...
            .unwrap_err();
        assert_eq!(error.code(), "model_request_cancelled");
