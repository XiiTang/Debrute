...
pub(super) fn execute(
    ...
    let delivery = arguments
        ...
        .ok_or_else(|| {
            ModelRequestError::new(
                ...
                "gemini-3-pro-image requires materialized delivery.",
            )
        })?;
    ...
    for field in ["model", "response_format", "store"] {
        if body.contains_key(field) {
            return Err(ModelRequestError::new(
                ...
                format!(
                    "gemini-3-pro-image arguments.{field} conflicts with Debrute request framing."
                ),
            ));
        }
    }
    if !input.is_empty() && body.contains_key("input") {
        return Err(ModelRequestError::new(
            "model_request_argument_collision",
            "gemini-3-pro-image cannot combine flattened prompt or image with arguments.input.",
        ));
    }
    body.insert(
        "model".to_owned(),
...
fn resolve_image(
    ...
    if project_media_kind_from_content_type(&mime) != CanvasMediaKind::Image {
        return Err(ModelRequestError::new(
            ...
            "gemini-3-pro-image input is not image media.",
        ));
    }
    Ok(json!({
        "type": "image",
...
fn parse_response(
    ...
    let steps = response
        ...
        .ok_or_else(|| {
            ModelRequestError::new(
                ...
                "gemini-3-pro-image response omitted steps.",
            )
        })?;
        ...
        .filter(|step| step.get("type").and_then(Value::as_str) == Some("model_output"))
    {
        let output_items = step
            ...
            .ok_or_else(|| {
                ModelRequestError::new(
                    "model_response_invalid",
                    "gemini-3-pro-image model_output step omitted content.",
                )
            })?;
            ...
            .filter(|item| item.get("type").and_then(Value::as_str) == Some("image"))
        {
            payloads.push(match delivery {
                ...
                _ => {
                    return Err(ModelRequestError::new(
                        "model_request_argument_invalid",
                        "gemini-3-pro-image delivery must be inline or uri.",
                    ));
                }
            });
        }
    ...
    if payloads.is_empty() {
        return Err(ModelRequestError::new(
            "model_response_invalid",
            "gemini-3-pro-image response contained no images.",
        ));
    }
    Ok(payloads)
}
...
fn payload_from_uri(
    context: &mut ExecutionContext<'_>,
    item: &Value,
) -> Result<crate::model_request::types::ModelArtifactPayload, ModelRequestError> {
    let uri = item
        .get("uri")
        ...
        .ok_or_else(|| {
            ModelRequestError::new(
                "model_response_invalid",
                "gemini-3-pro-image URI image omitted uri.",
            )
        })?;
    ...
    let mime = mime_from_path_or_bytes("", &response.body).ok_or_else(|| {
        ModelRequestError::new(
            ...
            "gemini-3-pro-image returned unsupported image bytes.",
        )
    })?;
...
fn payload_from_inline(
    ...
    let data = item
        ...
        .ok_or_else(|| {
            ModelRequestError::new(
                "model_response_invalid",
                "gemini-3-pro-image inline image omitted data.",
            )
        })?;
    let bytes = decode_base64(data, "gemini-3-pro-image image")?;
