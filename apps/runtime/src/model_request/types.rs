...
impl From<std::io::Error> for ModelRequestError {
    fn from(error: std::io::Error) -> Self {
        Self::new("model_request_filesystem_failed", error.to_string())
    }
}

#[derive(Debug, Clone, Default)]
pub(crate) struct ModelRequestCancellation(ModelCancellation);
...
impl ModelRequestCancellation {
    ...
    pub(crate) fn check(&self) -> Result<(), ModelRequestError> {
...
pub(crate) struct ModelRequestDeadline(Instant);

impl ModelRequestDeadline {
    pub(crate) fn after(timeout: Duration) -> Result<Self, ModelRequestError> {
        if timeout.is_zero() {
            return Err(ModelRequestError::new(
                "model_request_timeout_invalid",
                "Model request timeout must be a positive integer of milliseconds.",
            ));
        }
        Instant::now()
            .checked_add(timeout)
            ...
            .ok_or_else(|| {
                ModelRequestError::new(
                    "model_request_timeout_invalid",
                    "Model request timeout is outside the supported monotonic clock range.",
                )
            })
    }
    ...
    ) -> Result<Duration, ModelRequestError> {
        cancellation.check()?;
        self.0
...
#[derive(Debug, Clone)]
pub(crate) struct ModelArtifactPayload {
    pub bytes: Vec<u8>,
