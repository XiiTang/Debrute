...
use crate::model_request::image::{form_value, image_payload};
use crate::model_request::types::ModelExecutionDraft;
use crate::model_request::{
    common::{
        ExecutionContext, ResolvedMediaReference, decode_base64, join_url, mime_from_path_or_bytes,
        mime_from_response,
    },
    types::{HttpBody, HttpMethod, ModelRequestError, MultipartFile},
};
...
pub(super) fn execute(
    ...
    if arguments.contains_key("model") {
        return Err(ModelRequestError::new(
            ...
            "gpt-image-1 arguments.model conflicts with the configured request model ID.",
        ));
    }
    if arguments.contains_key("image") && arguments.contains_key("images") {
        return Err(ModelRequestError::new(
            "model_request_argument_collision",
            "gpt-image-1 cannot map both image and images.",
        ));
    }
...
) -> Result<(String, Value, Value), ModelRequestError> {
    body.insert(
        "model".to_owned(),
    ...
    let items = response
        ...
        .ok_or_else(|| {
            ModelRequestError::new(
                ...
                "gpt-image-1 response omitted a non-empty data array.",
            )
        })?;
    ...
    for item in items {
        let encoded = item
            ...
            .ok_or_else(|| {
                ModelRequestError::new(
                    "model_response_invalid",
                    "gpt-image-1 response item omitted b64_json.",
                )
            })?;
        let bytes = decode_base64(encoded, "gpt-image-1 image")?;
        let mime = mime_from_path_or_bytes("", &bytes).ok_or_else(|| {
            ModelRequestError::new(
                "model_artifact_type_unsupported",
                "gpt-image-1 returned an unsupported image encoding.",
            )
        })?;
...
impl ImageReference {
    ...
    fn public_url<'b>(
        &'b self,
        context: &mut ExecutionContext<'_>,
    ) -> Result<&'b str, ModelRequestError> {
        Ok(self
            .reference
    ...
    fn bytes(
                ...
                let mime = mime_from_response(&response)
                    ...
                    .ok_or_else(|| {
                        ModelRequestError::new(
                            ...
                            "gpt-image-1 edit input is not a supported image.",
                        )
                    })?;
                (mime, response.body)
            }
        ...
        if !mime.starts_with("image/") {
            return Err(ModelRequestError::new(
                "model_request_input_invalid",
                "gpt-image-1 edit input is not an image data URI.",
            ));
        }
        Ok((mime, bytes))
    }
...
fn resolve_images(
    context: &mut ExecutionContext<'_>,
    value: Option<Value>,
) -> Result<Vec<ImageReference>, ModelRequestError> {
    ...
    value
        ...
        .ok_or_else(|| {
            ModelRequestError::new(
                ...
                "gpt-image-1 image must be an array of strings.",
            )
        })?
        .iter()
...
fn resolve_mask(
    context: &mut ExecutionContext<'_>,
    value: Option<Value>,
) -> Result<Option<ImageReference>, ModelRequestError> {
    value
...
fn resolve_image(
    ...
    value: &Value,
    field: &str,
) -> Result<ImageReference, ModelRequestError> {
    let source = value.as_str().ok_or_else(|| {
        ModelRequestError::new(
            "model_request_argument_invalid",
            format!("gpt-image-1 {field} values must be strings."),
        )
    })?;
    Ok(ImageReference {
