...
fn audio_from_reference(
    ...
    if let Some((mime, encoded)) = inline_file(reference)? {
        return Ok((
            decode_base64(encoded, "fal Stable Audio 2.5 inline audio")?,
            mime.to_owned(),
        ));
    }
...
fn inline_file(reference: &str) -> Result<Option<(&str, &str)>, ModelRequestError> {
    ...
    let (metadata, encoded) = value.split_once(',').ok_or_else(|| {
        ModelRequestError::new(
            "model_response_invalid",
            "fal Stable Audio 2.5 returned a malformed data URI.",
        )
    })?;
    let mime = metadata
        .strip_suffix(";base64")
        ...
        .ok_or_else(|| {
            ModelRequestError::new(
                "model_response_invalid",
                "fal Stable Audio 2.5 returned a non-audio or non-base64 data URI.",
            )
        })?;
