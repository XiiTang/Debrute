...
use crate::model_request::image::image_payload;
use crate::model_request::types::ModelExecutionDraft;

const MODEL_ID: &str = "gemini-3.1-flash-image";
...
fn payload_from_inline(item: &Value) -> Result<ModelArtifactPayload, ModelRequestError> {
    let data = item
        .get("data")
        .and_then(Value::as_str)
        .filter(|value| !value.is_empty())
