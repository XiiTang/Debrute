...
Alibaba Cloud Model Studio documents `qwen-image-2.0-pro-2026-06-22` as the
current recommended fixed Pro snapshot. It combines text-to-image and image
editing in one model and emphasizes text rendering, realistic texture, detail,
and semantic adherence. Debrute binds this dated model ID instead of the
floating `qwen-image-2.0-pro` alias; the official model table currently maps
that alias to an older snapshot.
...
```json
{
  "model": "qwen-image-2.0-pro-2026-06-22",
  "arguments": {
    "prompt": "A premium editorial cover with exact bilingual typography and realistic materials"
  },
  "output": {
    "directory": "generated",
    "name": "image"
