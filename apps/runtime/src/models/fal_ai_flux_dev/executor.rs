...
fn image_from_reference(
    ...
    let bytes = decode_base64(encoded, "fal-ai/flux/dev inline image")?;
...
fn inline_file(reference: &str) -> Result<Option<(&str, &str)>, ModelRequestError> {
    ...
    let (metadata, encoded) = value.split_once(',').ok_or_else(|| {
        ModelRequestError::new(
            "model_response_invalid",
            "fal-ai/flux/dev returned a malformed data URI.",
        )
    })?;
    let mime = metadata
        .strip_suffix(";base64")
        ...
        .ok_or_else(|| {
            ModelRequestError::new(
                "model_response_invalid",
                "fal-ai/flux/dev returned a non-image or non-base64 data URI.",
            )
        })?;
