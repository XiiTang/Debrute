        ...
        match poll.get("status").and_then(Value::as_str) {
            ...
            Some("failed") => {
                ...
                return Err(ModelRequestError::new(
                    ...
                    format!("doubao-seedance-2-0-260128 task failed ({code}): {message}"),
                ));
            }
            Some("cancelled") => {
                return Err(ModelRequestError::new(
                    "model_request_task_failed",
                    "doubao-seedance-2-0-260128 task was cancelled by the remote endpoint.",
                ));
            }
            Some(status) => {
                return Err(ModelRequestError::new(
    ...
    let intent = arguments
        ...
        .ok_or_else(|| {
            ModelRequestError::new(
                ...
                "doubao-seedance-2-0-260128 requires a materialized string intent.",
            )
        })?;
...
fn normalize_reference(
        ...
        .find(|key| !matches!(key.as_str(), "source" | "media_type"))
    {
        return Err(ModelRequestError::new(
            "model_request_argument_invalid",
            format!(
                "doubao-seedance-2-0-260128 references[{index}] contains unknown field {unknown}."
            ),
        ));
    }
    ...
    let media_type = match reference.get("media_type") {
        ...
        Some(_) => {
            return Err(ModelRequestError::new(
                "model_request_argument_invalid",
                format!(
                    "doubao-seedance-2-0-260128 references[{index}].media_type must be a non-empty string."
                ),
            ));
        }
...
fn exact_task_failure(response: &Value) -> Result<(&str, &str), ModelRequestError> {
    let error = response
        ...
        .ok_or_else(|| {
            ModelRequestError::new(
                "model_response_invalid",
                "doubao-seedance-2-0-260128 failed without error.",
            )
        })?;
    let code = error
        .get("code")
        ...
        .ok_or_else(|| {
            ModelRequestError::new(
                "model_response_invalid",
                "doubao-seedance-2-0-260128 failed without error.code.",
            )
        })?;
    let message = error
        .get("message")
        ...
        .ok_or_else(|| {
            ModelRequestError::new(
                "model_response_invalid",
                "doubao-seedance-2-0-260128 failed without error.message.",
            )
        })?;
    Ok((code, message))
}
