    ...
    let result = (|| loop {
        ...
        match poll.pointer("/task/status").and_then(Value::as_str) {
            ...
            Some("succeeded") => {
                ...
                let payload = download_video_artifact(
                    context,
                    video_url,
...
fn request_body(context: &mut ExecutionContext<'_>) -> Result<Map<String, Value>, GenerationError> {
...
fn resolve_project_media(
    ...
    for item in items {
        ...
        for field in ["image_url", "video_url", "audio_url"] {
            ...
            if is_native_reference(&url) || !context.project_root.join(&url).is_file() {
                continue;
            }
...
fn task_url(base_url: &str, prefix: &str, task_id: &str) -> Result<String, GenerationError> {
    let prefix = join_url(base_url, prefix)?;
    let mut url = url::Url::parse(&prefix)
...
fn task_failure(response: &Value) -> GenerationError {
    let code = response
