...
fn request_body(context: &mut ExecutionContext<'_>) -> Result<Value, GenerationError> {
    ...
    let prompt = arguments.remove("prompt").ok_or_else(|| {
        GenerationError::new(
            ...
            "doubao-seedance-2-0-fast-260128 requires prompt.",
        )
    })?;
    ...
    let source = reference
        ...
        .ok_or_else(|| {
            GenerationError::new(
                "generation_argument_invalid",
                format!(
                    "doubao-seedance-2-0-fast-260128 references[{index}].source must be non-empty."
                ),
            )
        })?;
...
fn validate_intent(intent: &str, references: &[VideoReference]) -> Result<(), GenerationError> {
    ...
    if valid {
        ...
        Err(GenerationError::new(
            ...
            format!("doubao-seedance-2-0-fast-260128 references are invalid for intent {intent}."),
        ))
    }
}
...
fn remote_task_error(response: &Value) -> GenerationError {
    ...
    GenerationError::new(
        ...
        format!("doubao-seedance-2-0-fast-260128 task {status}: {detail}"),
    )
}
