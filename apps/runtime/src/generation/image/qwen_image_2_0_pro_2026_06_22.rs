...
pub(super) fn execute(context: &mut ExecutionContext<'_>) -> Result<ImageResult, GenerationError> {
    ...
    let prompt = parameters.remove("prompt").ok_or_else(|| {
        GenerationError::new(
            ...
            "qwen-image-2.0-pro-2026-06-22 requires prompt.",
        )
    })?;
        ...
        |value| {
            value
                ...
                .ok_or_else(|| {
                    GenerationError::new(
                        "generation_argument_invalid",
                        "qwen-image-2.0-pro-2026-06-22 image must be an array of strings.",
                    )
                })?
                .iter()
                .map(|value| {
                    let source = value.as_str().ok_or_else(|| {
                        GenerationError::new(
                            "generation_argument_invalid",
                            "qwen-image-2.0-pro-2026-06-22 image values must be strings.",
                        )
                    })?;
    ...
    let choices = response
        ...
        .ok_or_else(|| {
            GenerationError::new(
                ...
                "qwen-image-2.0-pro-2026-06-22 response omitted non-empty output.choices.",
            )
        })?;
    ...
    for choice in choices {
        let items = choice
            ...
            .ok_or_else(|| {
                GenerationError::new(
                    "model_response_invalid",
                    "qwen-image-2.0-pro-2026-06-22 response choice omitted non-empty message.content.",
                )
            })?;
        for item in items {
            urls.push(
                item.get("image")
                    ...
                    .ok_or_else(|| {
                        GenerationError::new(
                            "model_response_invalid",
                            "qwen-image-2.0-pro-2026-06-22 response content item omitted image.",
                        )
                    })?,
            );
        }
