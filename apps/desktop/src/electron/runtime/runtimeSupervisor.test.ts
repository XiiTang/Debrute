...
describe('RuntimeSupervisor', { tags: ['runtime'] }, () => {
  it('attaches to a healthy external runtime without marking it owned', async () => {
    ...
    const supervisor = new RuntimeSupervisor({
      ...
      launchRuntime: vi.fn(),
      checkHealth: healthCheck('healthy')
    });
  ...
  it('starts attached in degraded state when daemon is reachable but web is unavailable', async () => {
    ...
    const supervisor = new RuntimeSupervisor({
      ...
      terminateOwned: vi.fn(),
      launchRuntime: vi.fn(),
      checkHealth: healthCheck('web-unavailable')
    });
  ...
  it('terminates an owned runtime when Desktop launch fails health validation', async () => {
    ...
    const supervisor = new RuntimeSupervisor({
      ...
      checkHealth: healthCheck('daemon-unavailable')
    });
  ...
  it('stops only a runtime owned by this desktop session', async () => {
    ...
    const terminateOwned = vi.fn(async () => termination.promise);
    ...
    await supervisor.start();
    const stop = supervisor.stopOwnedRuntime();

    await vi.waitFor(() => expect(terminateOwned).toHaveBeenCalledWith(owned, owned.owner));
  ...
  it('surfaces runtime state read failures during Desktop stop', async () => {
    const owned = runtimeState({
      owner: { kind: 'desktop', ownerId: 'desktop-owner', pid: 222 }
    });
    const readFailure = Object.assign(new Error('state permission denied'), { code: 'EACCES' });
    const terminateOwned = vi.fn();
    const deleteState = vi.fn(async () => undefined);
    const supervisor = new RuntimeSupervisor({
      owner: owned.owner,
    ...
    await supervisor.start();
    await expect(supervisor.stopOwnedRuntime()).rejects.toBe(readFailure);
...
function healthCheck(status: WorkbenchRuntimeHealthStatus) {
