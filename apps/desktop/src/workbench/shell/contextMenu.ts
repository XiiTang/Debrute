import type { CanvasProjection, CanvasViewport, ProjectedCanvasNode } from '@axis/canvas-core';
...
export function centeredCanvasViewportForNode(input: {
  node: Pick<ProjectedCanvasNode, 'x' | 'y' | 'width' | 'height'>;
  surfaceSize: { width: number; height: number };
  viewport: Pick<CanvasViewport, 'zoom'>;
}): CanvasViewport {
  const nodeCenter = {
  ...
  return {
    x: input.surfaceSize.width / 2 - nodeCenter.x * input.viewport.zoom,
    y: input.surfaceSize.height / 2 - nodeCenter.y * input.viewport.zoom,
