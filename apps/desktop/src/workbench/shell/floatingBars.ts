import type { CanvasFeedbackEntry, CanvasViewport } from '@axis/canvas-core';
...
export function canvasNodeToViewportRect(input: {
  ...
  return {
    x: input.surfaceRect.x + input.viewport.x + input.nodeRect.x * input.viewport.zoom,
    y: input.surfaceRect.y + input.viewport.y + input.nodeRect.y * input.viewport.zoom,
    width: input.nodeRect.width * input.viewport.zoom,
    height: input.nodeRect.height * input.viewport.zoom
  };
}
