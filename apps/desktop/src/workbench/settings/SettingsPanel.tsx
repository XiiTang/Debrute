...
import { Bot, Cpu, DownloadCloud, Image as ImageIcon, KeyRound, RefreshCw, Save, Search, Terminal, Trash2, Wrench } from 'lucide-react';
import type {
  CanvasSettingsView,
...
import type { DesktopUpdateState, WorkbenchActions, WorkbenchState } from '../../types';
import { IntegrationsSettingsPage } from './integrations/IntegrationsSettingsPage';
import { CliSettingsPage } from './cli/CliSettingsPage';
...
const SETTINGS_NAV_ITEMS = [
  ...
  { id: 'cli', label: 'CLI', description: 'Command install and PATH', icon: Terminal },
  { id: 'updates', label: 'Updates', description: 'Version and release channel', icon: DownloadCloud }
] as const;
    ...
    <div className="settings-panel">
      ...
      <div className="settings-page">
        {activePage === 'llm' ? (
          ...
          <CliSettingsPage state={state} actions={actions} />
        ) : (
          <UpdatesSettingsPage updateState={state.updateState} onUpdateNow={actions.updateNow} />
        )}
      </div>
    </div>
  );
...
export function UpdatesSettingsPage({
  ...
  const state = updateState ?? { type: 'disabled' as const, reason: 'development' as const };
  const disabled = state.type === 'disabled' || state.type === 'checking' || state.type === 'downloading' || state.type === 'installing';
  return (
    <section className="settings-section update-settings-page">
      <SettingsSectionHeader title="Updates" eyebrow="Desktop" description="Check the installed build and run manual update checks." />
      <div className="settings-card update-settings-card">
        <strong>{currentVersionLabel(state)}</strong>
        <span>{updateStatusLabel(state)}</span>
        {'updateVersion' in state ? <small>Update version {state.updateVersion}</small> : null}
        {'lastError' in state && state.lastError ? <small className="update-settings-error">{state.lastError}</small> : null}
        <div className="settings-actions">
          <button type="button" disabled={disabled} onClick={() => void onUpdateNow()}>
            <RefreshCw size={14} />
            更新
          </button>
        </div>
...
function currentVersionLabel(state: DesktopUpdateState): string {
  return 'currentVersion' in state ? `Current version ${state.currentVersion}` : 'Current version unknown';
}

function updateStatusLabel(state: DesktopUpdateState): string {
  if (state.type === 'disabled') {
    return state.reason === 'development' ? 'Updates unavailable in development builds' : 'Updates unavailable';
  }
  if (state.type === 'checking') {
    return 'Checking for updates';
  }
  if (state.type === 'available') {
    return `Update available: ${state.updateVersion}`;
  }
    ...
    <article className="settings-card settings-model-card">
      <div className="settings-model-card-header">
        <div>
          ...
          <small>{model.provider} / {model.apiKeySet ? 'configured' : 'no key'}</small>
        </div>
      </div>
      <div className="settings-model-card-fields">
        ...
        <div className="settings-model-edit-grid">
          ...
          <div className="settings-row">
            <input
              aria-label="Provider Model ID override"
              value={draft.providerModelIdOverride}
              onChange={(event) => setDraft({ ...draft, providerModelIdOverride: event.currentTarget.value })}
              onBlur={() => void saveDraft(draft)}
              placeholder={model.defaultProviderModelId}
            />
          </div>
        </div>
      </div>
...
function modelDraftToSaveInput(draft: ModelDraft) {
  return {
    ...
    providerModelIdOverride: draft.providerModelIdOverride.trim() || null,
    ...(draft.apiKey.trim() ? { apiKey: draft.apiKey.trim() } : {})
  };
}
...
function modelDraftMatchesPersisted(draft: ModelDraft, model: ImageModelSettingRecord | VideoModelSettingRecord): boolean {
  return draft.baseUrlOverride.trim() === (model.baseUrlOverride ?? '')
    && draft.providerModelIdOverride.trim() === (model.providerModelIdOverride ?? '')
    && draft.apiKey.trim() === '';
}
