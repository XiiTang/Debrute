...
export function getCanvasById(snapshot: ProjectSessionSnapshot | undefined, canvasId: string | undefined): CanvasDocument | undefined {
...
export function projectRelativeSource(snapshot: ProjectSessionSnapshot | undefined, absolutePath: string): string {
  if (!snapshot || !absolutePath.startsWith(snapshot.projectRoot)) {
    return absolutePath;
  }
  return absolutePath.slice(snapshot.projectRoot.length + 1);
}
