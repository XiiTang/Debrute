...
describe('daemon project native file operations', () => {
  ...
  it('rejects symlink escapes before exposing absolute paths', async () => {
    ...
    const outsideRoot = await mkdtemp(join(tmpdir(), 'debrute-native-outside-'));
    try {
      await writeFile(join(outsideRoot, 'linked.txt'), 'outside', 'utf8');
      await symlink(
        outsideRoot,
      ...
      await expect(copyProjectAbsolutePaths({
        projectRoot,
        entries: [{ projectRelativePath: 'outside/linked.txt', kind: 'file' }]
