import type { AxisCliErrorCode } from '../errors/cliErrors.js';
...
export function renderAgentRecord(result: AxisAgentResult): string {
  const lines = result.status === 'ok'
    ? [`axis/1 ok cmd=${formatValue(result.command)}`]
    : [
        `axis/1 error cmd=${formatValue(result.command)} code=${formatValue(result.code)}`,
