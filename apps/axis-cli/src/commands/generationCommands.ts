...
import type { RunImageModelBatchInput } from '@axis/app-protocol';
...
export async function runGenerationCommand(args: ParsedAxisArgs, server: AxisAppServer): Promise<AxisAgentResult> {
  if (!args.projectRoot) {
...
function requestInput(args: ParsedAxisArgs): { model: string; arguments: Record<string, unknown>; timeoutMs?: number } {
...
function imageBatchInputFromArgs(args: ParsedAxisArgs): RunImageModelBatchInput {
...
function capabilityResult(command: string, result: Awaited<ReturnType<AxisAppServer['runImageModelRequestForCli']>>): AxisAgentResult {
...
function artifactRecords(artifacts: NonNullable<Awaited<ReturnType<AxisAppServer['runImageModelRequestForCli']>>['artifacts']>): AgentNamedRecord[] {
...
function positiveIntegerOption(args: ParsedAxisArgs, key: string, fallback: number): number {
  return optionalPositiveIntegerOption(args, key) ?? fallback;
}

function optionalPositiveIntegerOption(args: ParsedAxisArgs, key: string): number | undefined {
...
function nonNegativeIntegerOption(args: ParsedAxisArgs, key: string, fallback: number): number {
  const raw = args.options[key];
  if (raw === undefined) {
    return fallback;
  }
