...
import { createAxisDaemonHttpServer, type AxisDaemonHttpServer } from '@axis/daemon';
import { AxisCliError, exitCodeForCliError } from '../src/errors/cliErrors';
...
describe('axis workbench url CLI metadata', () => {
  it('parses workbench url with a project path', () => {
    const parsed = parseAxisArgs(['workbench', 'url', '.']);
  ...
  it('rejects unknown workbench url options and json output mode', () => {
    expect(() => parseAxisArgs(['workbench', 'url', '.', '--open'])).toThrow(/Unknown option/);
    expect(() => parseAxisArgs(['workbench', 'url', '.', '--json'])).toThrow(/--json is not supported/);
  });

  it('assigns runtime failures to configuration exit code', () => {
    expect(exitCodeForCliError(new AxisCliError('runtime_launch_failed', 'failed'))).toBe(3);
    expect(exitCodeForCliError(new AxisCliError('runtime_health_failed', 'failed'))).toBe(3);
    expect(exitCodeForCliError(new AxisCliError('runtime_state_unreadable', 'failed'))).toBe(3);
    expect(exitCodeForCliError(new AxisCliError('runtime_state_write_failed', 'failed'))).toBe(3);
    expect(exitCodeForCliError(new AxisCliError('runtime_lock_timeout', 'failed'))).toBe(3);
  });
});
...
describe('runWorkbenchCommand', () => {
  ...
  it('opens a project through the runtime daemon and returns URL fields', async () => {
    const projectRoot = await mkdtemp(join(tmpdir(), 'axis-workbench-url-project-'));
    cleanups.push(() => rm(projectRoot, { recursive: true, force: true }));

    const result = await runWorkbenchCommand(parseAxisArgs(['workbench', 'url', projectRoot]), {
      ensureRuntime: async () => ({
        runtimeStarted: false,
        statePath: '/home/user/.axis/runtime/workbench-runtime.json',
        state: runtimeState()
      }),
      fetch: async (url, init) => {
        expect(String(url)).toBe('http://127.0.0.1:17321/api/projects/open');
        expect(init?.method).toBe('POST');
        expect((init?.headers as Record<string, string>)['x-axis-daemon-token']).toBe('secret');
    ...
    expect(result).toMatchObject({
      ...
      command: 'workbench.url',
      fields: {
        project_url: 'http://127.0.0.1:17322/projects/project-1?axis-token=secret',
        ...
        state_path: '/home/user/.axis/runtime/workbench-runtime.json'
      }
    });
  ...
  it('reports desktop runtime kind when reusing a registered desktop runtime', async () => {
    const projectRoot = await mkdtemp(join(tmpdir(), 'axis-workbench-url-desktop-'));
    cleanups.push(() => rm(projectRoot, { recursive: true, force: true }));
  ...
  it('lets daemon project-open initialize missing AXIS metadata and default canvas', async () => {
    const projectRoot = await mkdtemp(join(tmpdir(), 'axis-workbench-url-init-'));
    ...
    const metadata = JSON.parse(await readFile(join(projectRoot, '.axis/project.json'), 'utf8')) as {
      project?: { name?: string };
    };
    expect(metadata.project?.name).toBe(projectRoot.slice(projectRoot.lastIndexOf('/') + 1));
    expect(await readdir(join(projectRoot, '.axis/canvases'))).toContain('production-map.json');
  });

  it('returns project_not_found for missing directories without launching runtime', async () => {
    const missingProject = join(tmpdir(), `axis-missing-${Date.now()}`);
    let launched = false;

    const result = await runWorkbenchCommand(parseAxisArgs(['workbench', 'url', missingProject]), {
      ensureRuntime: async () => {
        launched = true;
        return {
          runtimeStarted: false,
  ...
  it('preserves daemon internal_error from project-open failures', async () => {
    const projectRoot = await mkdtemp(join(tmpdir(), 'axis-workbench-url-invalid-'));
    cleanups.push(() => rm(projectRoot, { recursive: true, force: true }));
    ...
    const result = await runWorkbenchCommand(parseAxisArgs(['workbench', 'url', projectRoot]), {
      ...
      fetch: async () => new Response(JSON.stringify({
        error: {
          code: 'internal_error',
          message: 'Invalid AXIS project metadata.'
        }
      }), {
        status: 500,
...
describe('workbench runtime state and health', () => {
  ...
  it('returns undefined for missing runtime state', async () => {
    const root = await mkdtemp(join(tmpdir(), 'axis-runtime-state-missing-'));
    try {
      const paths = resolveWorkbenchRuntimePaths(root);
...
describe('ensureWorkbenchRuntime', () => {
  ...
  it('launches fresh state and terminates recorded stale pids when recorded state is stale', async () => {
    const root = await mkdtemp(join(tmpdir(), 'axis-runtime-launch-'));
  ...
  it('uses AXIS default workbench ports and moves occupied ports to a free loopback port', async () => {
  ...
  it('removes failed fresh runtime state and terminates launched pids when health fails', async () => {
    const root = await mkdtemp(join(tmpdir(), 'axis-runtime-health-fail-'));
    const kill = vi.spyOn(process, 'kill').mockImplementation(() => true);
    try {
      const paths = resolveWorkbenchRuntimePaths(root);
      const launched = runtimeState({
...
describe('internal workbench runtime child args', () => {
  it('resolves pkg entry directories from the executable when import.meta.url is unavailable', () => {
    ...
    expect(resolveWorkbenchRuntimeEntryDir?.(undefined, '/payload/axis')).toBe('/payload');
  });
  ...
  it('parses packaged runtime child args from environment variables', () => {
    expect(parseInternalWorkbenchRuntimeChildArgs({
      AXIS_WORKBENCH_RUNTIME_PORT: '17321',
      AXIS_WORKBENCH_RUNTIME_TOKEN: 'secret',
      AXIS_WORKBENCH_RUNTIME_WEB_DIST_DIR: '/payload/web'
    })).toEqual({
      port: 17321,
      token: 'secret',
