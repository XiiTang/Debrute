...
export const commandSpecs: DebruteCommandSpec[] = [
  ...
  spec('canvas.reset-layout', ['canvas', 'reset-layout'], 'project', 'write', 'project', 'canvas-map', '<project> <canvas-id> --all | <project> <canvas-id> --path <rule...>', 'Canvas layout reset record', [...PROJECT_LOAD_ERRORS, ...CANVAS_REGISTRY_ERRORS, 'canvas_map_invalid_path', 'canvas_map_invalid_yaml', 'canvas_map_canvas_missing']),
  ...
  spec('generate.image-batch', ['generate', 'image-batch'], 'generation', 'generate', 'project-session', 'assets', '<project> --manifest <path> --log <path> [--summary <path>] [--concurrency <n>] [--retries <n>] [--timeout-ms <ms>] [--overwrite-existing] | <project> --input-jsonl <path> --log <path> [--summary <path>] [--concurrency <n>] [--retries <n>] [--timeout-ms <ms>] [--overwrite-existing]', 'batch progress and summary records', [...PROJECT_LOAD_ERRORS, ...MODEL_RUNTIME_ERRORS]),
