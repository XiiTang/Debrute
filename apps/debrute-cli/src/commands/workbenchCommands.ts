...
interface DebruteHttpErrorBody {
  error?: {
    code?: unknown;
    message?: unknown;
  };
...
function requireProjectRoot(args: ParsedDebruteArgs): string {
  if (!args.projectRoot) {
...
async function openProject(
  ...
  try {
    ...
    if (!parsed.projectId) {
      throw cliError('runtime_health_failed', 'Debrute daemon project open response did not include projectId.');
    }
...
async function cliErrorFromProjectOpenResponse(response: Response) {
  ...
  if (!daemonError) {
    return cliError('runtime_health_failed', `Debrute daemon project open failed: ${response.status}`);
  }
...
async function readDaemonError(response: Response): Promise<{ code: string; message: string } | undefined> {
  try {
    const parsed = await response.json() as DebruteHttpErrorBody;
...
function workbenchProjectUrl(webUrl: string, projectId: string, token: string): string {
  ...
  url.searchParams.set('debrute-token', token);
  return url.toString();
}
