...
Canvas Feedback is Project-scoped current review state for Project Nodes. It is
...
Documents so the same Project Node keeps one review state across Canvas views
...
more exact Project Node paths and sets or clears one Mark for all of them in one
...
- a node-scoped comment on any Project Node;
...
Hovering one node opens the shared full floating Feedback Bar. Every Project
...
and locks its target to the Capsule's Project Node. Pointer movement cannot hide
...
Project Node, the Bar switches directly to that Node after focus leaves; it
...
focused Capsule, leaving either the current Project Node or the Bar starts the
same 120-millisecond dismissal grace. Entering a Project Node or re-entering the
