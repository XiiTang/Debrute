...
versioned product. The Product Installer carries a complete Product seed and
invokes Runtime-owned installation, which materializes the matching Runtime
Product under the user product root and selects one active version through the
stable `current` path defined by
...
whole-Product update capability. The managed CLI is replaced as part of that
