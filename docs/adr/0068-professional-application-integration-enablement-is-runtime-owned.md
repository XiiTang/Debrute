# Professional Application Integration Enablement Is Runtime Owned

Each supported Professional Application Integration has one explicit,
default-off field in Runtime-owned Global Settings. Photoshop is the first:
`integrations.photoshop.enabled`. Workbench Settings submits a closed mutation and
...
There is no Integration master switch. When every explicit Integration is off,
the integration surface is operationally off because no plugin listener,
route, session, credential, or command authority exists. A future Integration
adds a reviewed peer field and its own lifecycle rather than entering an
arbitrary registry or inheriting the Photoshop protocol.
...
Plugin continues bounded discovery while disconnected, including while the
Photoshop Integration is off, so reconnection is automatic when both sides are
...
turning Runtime shutdown, Product update, or unrelated work into Integration busy
state.
