...
Canvas feedback bars use the Canvas chrome layer and shared controls. Their size is derived from the visible fixed-size actions, creator, and saved-item row rather than media-specific width buckets. Fixed spatial tools remain a semantic group but receive no surrounding visual frame, fill, divider, or spacing treatment that distinguishes them from configured Mark actions. Inside the Bar, the creator and editable Feedback Capsules use one Canvas-owned square technical frame rather than an offset control underlayer: a muted one-pixel idle frame becomes the shared two-pixel focus frame without changing geometry, and video-moment idle frames retain their semantic moment color. Canvas has no persistent node-wide feedback border; feedback kinds remain visible in the editing bar and media annotations.

Feedback Settings reuses the complete image Feedback Bar presentation as its configuration preview so the preview cannot invent a second size or grouping treatment. Its fixed pin, rectangle, and Comment controls are visually exact but inert. Catalog mappings use one square technical frame around the icon and immutable user Name, wrap naturally from left to right, and indicate Bar membership with only the selection surface. The overlaid delete control does not participate in frame width. The blank first creation frame has no icon, label, plus sign, or nested frame.
